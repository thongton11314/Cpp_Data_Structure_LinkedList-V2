...
template<typename T>
void List<T>::doSomethingFun() {
    if (isEmpty()) return;
    Node* current = head;
    while (current->next != nullptr) {
        Node* ptr = new Node;
        *ptr = *current;
        //Node* ptr = new Node{ new T(*current->data), current->next };
        current->next = ptr;

        // this will end up crash when make empty call 
        // because last node never set = null
